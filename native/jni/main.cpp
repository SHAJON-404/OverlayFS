...
// MS_NOATIME, MS_NODIRATIME, MS_RELATIME and MS_STRICTATIME are mutually exclusive;
// keep only MS_NOATIME (suppresses all atime updates — safest for system partitions).
#define MNT_FLAGS (MS_LAZYTIME | MS_NOSUID | MS_NODEV | MS_NOEXEC | MS_DIRSYNC | MS_NOATIME | MS_NOSYMFOLLOW | MS_MANDLOCK | MS_SILENT)
...
int main(int argc, const char **argv) {
    ...
    }
    {
        ...
        if (version_fd >= 0) {
            ssize_t n = read(version_fd, buf, sizeof(buf)-1);
            if (n > 0) buf[n] = '\0';
            close(version_fd);
        }
        if (buf[0] != '\0') {
